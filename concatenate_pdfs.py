...
def normalize_pdf_pages(input_path, a4_width=595, a4_height=842):
    """Normalize pages to standard size"""
    ...
    for page in reader.pages:
        ...
        # Check if page is significantly larger than A4 (scale if needed)
        if width > a4_width * 1.1 or height > a4_height * 1.1:
            # Calculate scale factor to fit within A4
            scale_x = a4_width / width
            scale_y = a4_height / height
            scale = min(scale_x, scale_y) * 1  # 5% margin for safety
            
            # Scale the page using transformation matrix
    ...
    if modified:
        # Create temporary file for the normalized PDF
    ...
    return input_path  # Return original path if no changes needed
